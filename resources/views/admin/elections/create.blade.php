...
<div style="background:white;border-radius:8px;box-shadow:0 1px 4px rgba(0,0,0,0.08);overflow:hidden;">
    <div style="display:flex;align-items:center;gap:8px;padding:16px 20px;border-bottom:1px solid #f0f0f0;">
        ...
        <h2 style="font-size:1rem;font-weight:600;color:#333;margin:0;">Create New Election</h2>
    </div>
        ...
        <form method="POST" action="{{ route('admin.elections.store') }}">
            ...
            <div style="margin-bottom:16px;">
                ...
                <input type="text" name="title" value="{{ old('title') }}"
                       ...
                       style="width:100%;padding:9px 14px;border:1px solid {{ $errors->has('title') ? '#c0392b' : '#ddd' }};border-radius:6px;font-size:0.875rem;outline:none;box-sizing:border-box;"
            ...
            <div style="margin-bottom:16px;">
                ...
                <textarea name="description" rows="3"
                          ...
                          onfocus="this.style.borderColor='#c0392b'" onblur="this.style.borderColor='#ddd'">{{ old('description') }}</textarea>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:20px;">
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date') }}"
                           ...
                           required>
                    @error('start_date')<p style="color:#c0392b;font-size:0.78rem;margin:4px 0 0;">{{ $message }}</p>@enderror
                </div>
                <div>
                    ...
                    <input type="date" name="end_date" value="{{ old('end_date') }}"
                           style="width:100%;padding:9px 14px;border:1px solid #ddd;border-radius:6px;font-size:0.875rem;outline:none;box-sizing:border-box;"
                           required>
                    @error('end_date')<p style="color:#c0392b;font-size:0.78rem;margin:4px 0 0;">{{ $message }}</p>@enderror
                </div>
            </div>
<div style="margin-bottom:16px;">
